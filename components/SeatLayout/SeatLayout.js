...
export default function SeatLayout({
  ...
  const toggleSeatSelection = (rowIndex, column, index) => {
    setArr((prevArr) => {
      ...
      const targetColumn = column === "columnOne" ? "columnOne" : "columnTwo";
      const currentStatus = updatedArr[rowIndex][targetColumn][index];
      
      updatedArr[rowIndex][targetColumn][index] =
        currentStatus === "seat"
          ? "selectedSeat"
    ...
    const selectedSeatId = `${rowIndex}-${column}-${index}`;
    onSeatSelect(selectedSeats.includes(selectedSeatId) ? { id: selectedSeatId } : { id: selectedSeatId });
  };
        ...
        <View style={styles.driverContainer}>
          <Image
            ...
            style={[
              ...
              {
                alignSelf: driverPosition === "left" ? "flex-start" : "flex-end",
              },
            ]}
          />
        </View>
              ...
              >
                {item.columnOne.map((seat, index) => (
                  <TouchableOpacity
                    ...
                    onPress={() => toggleSeatSelection(rowIndex, "columnOne", index)}
                  >
                    <Image
                      source={
                        seat === "seat"
                      ...
                      style={{
                        height: seat === "seat" || seat === "selectedSeat" ? 36 : 56,
                        width: seat === "seat" || seat === "selectedSeat" ? 36 : 36,
                        margin: 2,
                      }}
                    />
                  </TouchableOpacity>
              ...
              >
                {item.columnTwo.map((seat, index) => (
                  <TouchableOpacity
                    key={index}
                    style={{ paddingVertical: isSleeper ? 4 : 2 }}
                    onPress={() => toggleSeatSelection(rowIndex, "columnTwo", index)}
                  >
                    <Image
                      source={
                        seat === "seat"
